...
int main(int argc, char *argv[])
{
	...
	char privatefile[] = "rsaprivatekey.pem";
	char publicfile[] = "rsappublickey.pem";

	char* buffer[1024];

	BIO *binfile, *boutfile, *privatekey,*publickey, *hash;
	binfile = BIO_new_file(infilename, "r");
	boutfile = BIO_new_file(outfilename, "w") ;
	privatekey = BIO_new_file(privatefile,"r");
	publickey = BIO_new_file(publicfile,"r");
	hash = BIO_new(BIO_f_md());
	BIO_set_md(hash, EVP_sha1());

	// Part C ===================================================
	// RSA *PEM_read_bio_RSAPrivateKey( BIO *bp, RSA **x, pem_password_cb *cb, void *u);
	RSA *rsa  = PEM_read_bio_RSAPrivateKey(privatekey, NULL, 0, NULL);
	...
	int flen = 128;
 	...
 	RSA_private_encrypt(flen, buff_from, buff_to, rsa, RSA_PKCS1_PADDING);

	// Part E ===================================================
	// RSA *PEM_read_bio_RSA_PUBKEY(BIO *bp, RSA **x, pem_password_cb *cb, void *u);
	RSA *rsa2  = PEM_read_bio_RSA_PUBKEY(publickey, NULL, 0, NULL);
