...
//	SSL_CTX_set_options(ctx, SSL_OP_ALL|SSL_OP_NO_SSLv2);
  ...
  // SSL_read(SSL *ssl, void *buf, int num)
	...
	memset(recieved,0,BUFFER_SIZE);

	// should read from SSL to write it to recieved
	SSL_read(ssl,recieved,BUFFER_SIZE);
	...
	//BIO_new(BIO_s_mem())
	//Creates new BIO
	BIO *newBIO = BIO_new(BIO_s_mem());
	//BIO_write
	BIO_write(newBIO,recieved,BUFFER_SIZE);
	//BIO_new_file
	BIO *newfile = BIO_new_file(generated_key.c_str(),"r");
	//PEM_read_bio_RSA_PUBKEY
	RSA *rsaPub = PEM_read_bio_RSA_PUBKEY(newfile,NULL,NULL,NULL);
	//RSA_public_decrypt
  //RSA_public_decrypt(int flen, unsigned char *from,unsigned char *to, RSA *rsa, int padding);
	int checking =   RSA_public_decrypt(RSA_size(rsaPub), (const unsigned char* )buff, (unsigned char* )recieved, rsaPub, RSA_PKCS1_PADDING);
	// If should be able to read something in if it is one it will be an error
	if( checking == -1 )
	{
		cout << "ERROR! Did not Authenticate" << endl;
		return 0;
	}
	//BIO_free
	BIO_free(newfile);	
